...
seastar::net::inet_address::parse_numerical(const sstring& addr) {
    ...
    // "<ipv6>%<zone>" (RFC 4007): the zone is an interface name or index. A
    // name that matches no interface makes the whole literal invalid, while
    // a numeric zone stands on its own so a literal from another host or a
    // configuration file keeps its zone even if no such interface exists here.
...
seastar::ipv6_addr::ipv6_addr(const std::string& s)
    : ipv6_addr([&] {
        // Without brackets the whole string is the address: "::1:9092" is a
        // valid address, so a port can only follow a bracketed one.
        if (s.empty() || s.front() != '[') {
