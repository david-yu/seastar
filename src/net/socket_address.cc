...
bool socket_address::operator==(const socket_address& a) const noexcept {
    ...
    // The zone (sin6_scope_id) is deliberately not compared, so an address
    // parsed with %zone equals the same address learned from the kernel
    // without one; std::hash<socket_address> agrees.
