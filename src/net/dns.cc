...
public:
    ...
    std::string message(int error) const {
        // c-ares owns the table; the hand-written copy this replaced had
        // stopped at ARES_ECANCELLED and rendered newer codes such as
        // ARES_ENOSERVER as "Unknown error".
...
dns_resolver::impl::get_host_by_name(sstring name, opt_family family)  {
    ...
    // Numeric literals never go to c-ares: it cannot parse a scope id
    // ("fe80::1%eth0"), and it hands back an IPv4 node even when AF_INET6
    // was requested. The requested family is a filter, so a literal of the
    // other family is an error, not a fallback.
