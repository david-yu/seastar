...
static future<> serve_udp_dns_responses(net::datagram_channel& chan, size_t n_queries) {
...
static future<hostent> ask_ipv6_udp_nameserver(size_t n_queries, Query query) {
    auto chan = make_bound_datagram_channel(socket_address(ipv6_addr{"::1", 0}));
    auto server = serve_udp_dns_responses(chan, n_queries);
...
// Not ::1, which /etc/hosts answers before c-ares ever asks the nameserver.
SEASTAR_TEST_CASE(test_reverse_lookup_ipv6_from_ipv6_nameserver) {
    if (!seastar::testing::ipv6_available_or_skip()) {
        co_return;
    }
    auto h = co_await ask_ipv6_udp_nameserver(1, [](dns_resolver& d) {
        return d.get_host_by_addr(inet_address("2001:db8::42"));
    });
